...
shinyServer(function(input, output, session) {
  ...
  observeEvent(c(input$submit, input$load_example), {
    ...
    if (input$submit){
      if (!is.null(input$file1)) {
        ...
        system(
          'python rnaCARD.py -i www/working_dir/input_file_rnaCARD.txt --match --os'
        )
  ...
  observeEvent(input$mode, {
    if (input$mode == "similar motifs") {
      ...
      shinyjs::hide(id = 'motif_count_header')
  ...
  create_table <- function(selected_motifs) {
    if (input$mode == "similar motifs") {
      ...
      output$table_motifs <-
        renderDT(
          datatable(
            as.data.frame(selected_motifs)[c(2, 3, 4, 5, 6)],
            class = 'table-light',
            options = list(pageLength = 15) ,
            selection = list(mode = 'single', selected = c(1)),
            filter = 'none',
  ...
  parseResults <- function(example = FALSE){
    ...
    if (example == FALSE){
      card_out <-
        read_delim(
          "www/working_dir/matched_whole_transcripts.txt",
          "\t",
          escape_double = FALSE,
          col_names = FALSE,
          trim_ws = TRUE
